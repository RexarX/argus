...
public:
   ...
   * @warning Triggers assertion if `N-1 > StrCapacity` (excluding null
   * terminator).
   ...
   * @warning Triggers assertion if `std::distance(first, last) > StrCapacity`.
  ...
  /**
   * @brief Constructs from a range using `std::from_range_t`.
   * @warning Triggers assertion if range size > `StrCapacity`.
   * @tparam Range Range type
   * @param tag `std::from_range` tag to disambiguate from iterator constructor
   ...
   * @tparam OtherCapacity `StrCapacity` of source string (must be less than
   * `StrCapacity`)
   ...
   * @warning Triggers assertion if `(pos + count) > other.Size()` or `count >
   * StrCapacity`.
   ...
   * @param count Number of characters to copy (default: all remaining)
   */
  constexpr BasicStaticString(BasicStaticString&& other, size_type pos,
                              size_type count = npos) noexcept;
  ...
  template <size_t OtherCapacity>
    requires(OtherCapacity < StrCapacity)
  constexpr BasicStaticString(
      const BasicStaticString<OtherCapacity, CharT, Traits>& other,
      size_type pos, size_type count = npos) noexcept;
  ...
  template <size_t OtherCapacity>
    requires(OtherCapacity < StrCapacity)
  constexpr BasicStaticString(
      BasicStaticString<OtherCapacity, CharT, Traits>&& other, size_type pos,
      size_type count = npos) noexcept;

  constexpr ~BasicStaticString() noexcept = default;

  constexpr BasicStaticString& operator=(const BasicStaticString&) noexcept =
      default;
  constexpr BasicStaticString& operator=(BasicStaticString&&) noexcept =
      default;
  ...
  template <size_t OtherCapacity>
    requires(OtherCapacity < StrCapacity)
  constexpr BasicStaticString& operator=(
      const BasicStaticString<OtherCapacity, CharT, Traits>& other) noexcept;
  ...
  /**
   * @brief Assigns from a `std::basic_string_view`.
   * @warning Triggers assertion if `sv.size() > StrCapacity`.
   * @param sv Source string view
   * @return Reference to this
   */
  ...
  /**
   * @brief Inserts a `std::basic_string_view` at position.
   * @warning Triggers assertion if `pos > Size()` or resulting size >
   * `Capacity()`.
   * @param pos Position to insert at
   * @param sv String view to insert
   * @return Reference to this
   */
  constexpr BasicStaticString& Insert(
      size_type pos, std::basic_string_view<CharT, Traits> sv) noexcept;
   ...
   * @return Reference to this
   */
  constexpr BasicStaticString& Insert(size_type pos, size_type count,
                                      CharT ch) noexcept;
  ...
  template <std::ranges::input_range Range>
    requires std::convertible_to<std::ranges::range_value_t<Range>, CharT>
  constexpr BasicStaticString& InsertRange(size_type pos,
   ...
   * @return Reference to this
   */
  constexpr BasicStaticString& Erase(size_type pos = 0,
                                     size_type count = npos) noexcept;
  ...
  /**
   * @brief Replaces characters in range `[pos, pos + count)` with
   ...
   * @return Reference to this
   */
  constexpr BasicStaticString& Replace(
      size_type pos, size_type count,
      std::basic_string_view<CharT, Traits> sv) noexcept;

  /**
   * @brief Replaces characters in range `[pos, pos + count)` with `count2`
   * copies of `ch`.
   * @warning Triggers assertion if `pos > Size()` or resulting size >
   * `Capacity()`.
   * @param pos Starting position
  ...
  constexpr BasicStaticString& Replace(size_type pos, size_type count,
                                       size_type count2, CharT ch) noexcept;

  /**
   * @brief Replaces characters in range `[pos, pos + count)` with characters
   * from a range.
   * @warning Triggers assertion if `pos > Size()` or resulting size >
   * `Capacity()`.
   * @tparam Range Range type
   * @param pos Starting position
   * @param count Number of characters to replace
   * @param range Range of replacement characters
   * @return Reference to this
   */
  template <std::ranges::input_range Range>
    requires std::convertible_to<std::ranges::range_value_t<Range>, CharT>
  constexpr BasicStaticString& ReplaceWithRange(size_type pos, size_type count,
                                                Range&& range) noexcept;
   ...
   */
  [[nodiscard]] constexpr BasicStaticString Substr(
   ...
   */
  [[nodiscard]] constexpr size_type RemainingCapacity() const noexcept {
    return StrCapacity - size_;
  }
...
private:
  std::array<CharT, StrCapacity + 1>
      data_{};          ///< Character storage (+ 1 for null terminator)
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    std::basic_string_view<CharT, Traits> sv) noexcept {
  ARGUS_ASSERT(sv.size() <= StrCapacity, "String view size exceeds capacity!");
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    std::basic_string_view<CharT, Traits> sv, size_type pos,
    size_type count) noexcept {
  ARGUS_ASSERT(pos <= sv.size(), "Position out of range!");
  const size_type substr_len = std::min(count, sv.size() - pos);
  ARGUS_ASSERT(substr_len <= StrCapacity, "Substring size exceeds capacity!");
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ARGUS_ASSERT(copy_len <= StrCapacity,
               "String literal size exceeds capacity!");
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ARGUS_ASSERT(count <= StrCapacity, "Count exceeds capacity!");
  std::ranges::fill_n(data_.begin(),
                      static_cast<std::iter_difference_t<CharT*>>(count), ch);
  size_ = count;
  data_[size_] = CharT{};
}
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ARGUS_ASSERT(len <= StrCapacity, "Length exceeds capacity!");
...
template <std::ranges::input_range Range>
  requires std::convertible_to<std::ranges::range_value_t<Range>, CharT>
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    std::from_range_t /*tag*/, Range&& range) noexcept {
  size_type count = 0;
  for (auto&& ch : std::forward<Range>(range)) {
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
  ...
  ARGUS_ASSERT(ilist.size() <= StrCapacity,
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::operator=(
    const CharT* str) noexcept -> BasicStaticString& {
  Assign(str);
  return *this;
}
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Insert(
    size_type pos, std::basic_string_view<CharT, Traits> sv) noexcept
    -> BasicStaticString& {
  ARGUS_ASSERT(pos <= size_, "Position out of range!");
  ARGUS_ASSERT(size_ + sv.size() <= StrCapacity,
               "Insert would exceed capacity!");

  // Move existing characters to make room
  if (pos < size_) {
    Traits::move(data_.data() + pos + sv.size(), data_.data() + pos,
                 size_ - pos);
  }
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Insert(
    size_type pos, size_type count, CharT ch) noexcept -> BasicStaticString& {
  ARGUS_ASSERT(pos <= size_, "Position out of range!");
  ARGUS_ASSERT(size_ + count <= StrCapacity, "Insert would exceed capacity!");

  if (pos < size_) {
    Traits::move(data_.data() + pos + count, data_.data() + pos, size_ - pos);
  }

  std::ranges::fill_n(data_.begin() + pos,
                      static_cast<std::iter_difference_t<CharT*>>(count), ch);
  size_ += count;
  data_[size_] = CharT{};
  return *this;
...
template <std::ranges::input_range Range>
  requires std::convertible_to<std::ranges::range_value_t<Range>, CharT>
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::InsertRange(
    size_type pos, Range&& range) noexcept -> BasicStaticString& {
  ARGUS_ASSERT(pos <= size_, "Position out of range!");

  // Calculate range size if possible
  size_type range_size = 0;
  if constexpr (std::ranges::sized_range<Range>) {
    range_size = static_cast<size_type>(std::ranges::size(range));
    ARGUS_ASSERT(size_ + range_size <= StrCapacity,
                 "Insert would exceed capacity!");

    // Move existing characters
    if (pos < size_) {
      Traits::move(data_.data() + pos + range_size, data_.data() + pos,
                   size_ - pos);
    }
    ...
    auto insert_pos = pos;
    for (auto&& ch : std::forward<Range>(range)) {
      ARGUS_ASSERT(size_ < StrCapacity, "Insert would exceed capacity!");
      if (insert_pos < size_) {
        Traits::move(data_.data() + insert_pos + 1, data_.data() + insert_pos,
                     size_ - insert_pos);
      }
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Erase(
    size_type pos, size_type count) noexcept -> BasicStaticString& {
  ARGUS_ASSERT(pos <= size_, "Position out of range!");
  const size_type erase_count = std::min(count, size_ - pos);
  const size_type remaining = size_ - pos - erase_count;
  if (remaining > 0) {
    Traits::move(data_.data() + pos, data_.data() + pos + erase_count,
                 remaining);
  }
  size_ -= erase_count;
  data_[size_] = CharT{};
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr void BasicStaticString<StrCapacity, CharT, Traits>::PushBack(
    CharT ch) noexcept {
  ARGUS_ASSERT(size_ < StrCapacity, "Cannot PushBack: string is at capacity!");
  data_[size_++] = ch;
  data_[size_] = CharT{};
}
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr void
BasicStaticString<StrCapacity, CharT, Traits>::PopBack() noexcept {
  ARGUS_ASSERT(!Empty(), "Cannot PopBack: string is empty!");
  data_[--size_] = CharT{};
}
...
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Append(
  ...
  ARGUS_ASSERT(size_ + count <= StrCapacity, "Append would exceed capacity!");
...
template <std::ranges::input_range Range>
  requires std::convertible_to<std::ranges::range_value_t<Range>, CharT>
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::AppendRange(
    Range&& range) noexcept -> BasicStaticString& {
  for (auto&& ch : std::forward<Range>(range)) {
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Replace(
    size_type pos, size_type count, size_type count2, CharT ch) noexcept
    -> BasicStaticString& {
  ARGUS_ASSERT(pos <= size_, "Position out of range!");
  const size_type replace_count = std::min(count, size_ - pos);
  ...
  if (tail_len > 0 && count2 != replace_count) {
    Traits::move(data_.data() + pos + count2, data_.data() + tail_pos,
                 tail_len);
  }

  std::ranges::fill_n(data_.begin() + pos,
                      static_cast<std::iter_difference_t<CharT*>>(count2), ch);
  size_ = new_size;
  data_[size_] = CharT{};
  return *this;
}
...
template <std::ranges::input_range Range>
  requires std::convertible_to<std::ranges::range_value_t<Range>, CharT>
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::ReplaceWithRange(
    size_type pos, size_type count, Range&& range) noexcept
    -> BasicStaticString& {
  ARGUS_ASSERT(pos <= size_, "Position out of range!");
  ...
  if constexpr (std::ranges::sized_range<Range>) {
    ...
    if (tail_len > 0 && range_size != replace_count) {
      Traits::move(data_.data() + pos + range_size, data_.data() + tail_pos,
                   tail_len);
    }
    ...
    temp.AppendRange(std::forward<Range>(range));
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Copy(
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr void BasicStaticString<StrCapacity, CharT, Traits>::Resize(
    size_type count, CharT ch) noexcept {
  ARGUS_ASSERT(count <= StrCapacity, "Resize count exceeds capacity!");
  if (count > size_) {
    std::ranges::fill_n(
        data_.begin() + size_,
        static_cast<std::iter_difference_t<CharT*>>(count - size_), ch);
  }
  size_ = count;
  data_[size_] = CharT{};
}
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr void BasicStaticString<StrCapacity, CharT, Traits>::Swap(
    BasicStaticString& other) noexcept {
...
template <std::ranges::input_range Range>
  requires std::convertible_to<std::ranges::range_value_t<Range>, CharT>
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::AssignRange(
    Range&& range) noexcept -> BasicStaticString& {
  Clear();
  return AppendRange(std::forward<Range>(range));
}
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::At(
    size_type pos) noexcept -> reference {
  ARGUS_ASSERT(pos < size_, "Position out of range!");
  return data_[pos];
}
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Front() noexcept
    -> reference {
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Front()
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Back() noexcept
    -> reference {
  ARGUS_ASSERT(!Empty(), "String is empty!");
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Back()
    const noexcept -> const_reference {
  ARGUS_ASSERT(!Empty(), "String is empty!");
  return data_[size_ - 1];
}
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr auto BasicStaticString<StrCapacity, CharT, Traits>::Substr(
    size_type pos, size_type count) const noexcept -> BasicStaticString {
  ARGUS_ASSERT(pos <= size_, "Position out of range!");
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    const BasicStaticString& other, size_type pos, size_type count) noexcept {
  ARGUS_ASSERT(pos <= other.Size(), "Position out of range!");
...
template <size_t StrCapacity, typename CharT, typename Traits>
  requires(StrCapacity > 0)
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    BasicStaticString&& other, size_type pos, size_type count) noexcept {
  ARGUS_ASSERT(pos <= other.Size(), "Position out of range!");
  const size_type substr_len = std::min(count, other.Size() - pos);
  ARGUS_ASSERT(substr_len <= StrCapacity, "Substring size exceeds capacity!");
  Traits::copy(data_.data(), other.Data() + pos, substr_len);
...
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    const BasicStaticString<OtherCapacity, CharT, Traits>& other) noexcept {
  ARGUS_ASSERT(other.Size() <= StrCapacity,
               "Source string size exceeds capacity!");
...
template <size_t OtherCapacity>
  requires(OtherCapacity < StrCapacity)
constexpr BasicStaticString<StrCapacity, CharT, Traits>::BasicStaticString(
    const BasicStaticString<OtherCapacity, CharT, Traits>& other, size_type pos,
    size_type count) noexcept {
  ARGUS_ASSERT(pos <= other.Size(), "Position out of range!");
  const size_type substr_len = std::min(count, other.Size() - pos);
  ARGUS_ASSERT(substr_len <= StrCapacity, "Substring size exceeds capacity!");
...
[[nodiscard]] constexpr auto operator+(
    const BasicStaticString<N1, CharT, Traits>& lhs,
...
/**
 * @brief Concatenates `StaticString` with `std::basic_string_view`.
...
[[nodiscard]] constexpr auto operator+(
    const BasicStaticString<N, CharT, Traits>& lhs,
...
/**
 * @brief Concatenates `std::basic_string_view` with `StaticString`.
 * @warning Triggers assertion if combined length > `N`.
 * @tparam N `StrCapacity` of the static string
 * @tparam CharT Character type
 * @tparam Traits Character traits
...
template <size_t N, typename CharT, typename Traits>
[[nodiscard]] constexpr auto operator+(
    const BasicStaticString<N, CharT, Traits>& lhs, CharT rhs) noexcept
    -> BasicStaticString<N, CharT, Traits> {
  BasicStaticString<N, CharT, Traits> result = lhs;
  result.PushBack(rhs);
  return result;
...
template <size_t N, typename CharT, typename Traits>
[[nodiscard]] constexpr auto operator+(
    CharT lhs, const BasicStaticString<N, CharT, Traits>& rhs) noexcept
    -> BasicStaticString<N, CharT, Traits> {
...
template <size_t StrCapacity, typename CharT, typename Traits>
constexpr void swap(
    BasicStaticString<StrCapacity, CharT, Traits>& lhs,
 ...
 */
template <size_t StrCapacity, typename CharT, typename Traits>
constexpr auto erase(BasicStaticString<StrCapacity, CharT, Traits>& str,
...
constexpr auto erase_if(BasicStaticString<StrCapacity, CharT, Traits>& str,
                        Pred pred) noexcept ->
    typename BasicStaticString<StrCapacity, CharT, Traits>::size_type {
  const auto original_size = str.Size();
 ...
 */
template <size_t StrCapacity, typename CharT, typename Traits>
inline auto operator>>(std::basic_istream<CharT, Traits>& is,
 ...
 */
template <size_t StrCapacity, typename CharT, typename Traits>
struct hash<argus::container::BasicStaticString<StrCapacity, CharT, Traits>> {
  [[nodiscard]] constexpr size_t operator()(
      const argus::container::BasicStaticString<StrCapacity, CharT, Traits>&
          str) const noexcept {
...
template <size_t StrCapacity, typename CharT, typename Traits>
struct formatter<
    argus::container::BasicStaticString<StrCapacity, CharT, Traits>>
  ...
  auto format(const argus::container::BasicStaticString<StrCapacity, CharT,
                                                        Traits>& str,
