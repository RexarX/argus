...
function(argus_target_set_warnings TARGET)
  set(MSVC_WARNINGS
  ...
  # When AdaptiveCpp (SYCL) is in use, the compiler may report as GNU (g++ launcher)
  # but internally delegate to a Clang-based backend that does not recognize
  # GCC-specific warning flags. Skip those to avoid -Wunknown-warning-option errors.
  ...
  if(ARGUS_ENABLE_WARNINGS_AS_ERRORS)
...
function(argus_target_set_optimization TARGET)
  target_compile_options(${TARGET} PRIVATE
...
function(argus_target_enable_lto TARGET)
...
function(argus_target_set_platform TARGET)
  target_compile_definitions(${TARGET} PUBLIC
      $<$<PLATFORM_ID:Windows>:ARGUS_PLATFORM_WINDOWS>
      $<$<PLATFORM_ID:Linux>:ARGUS_PLATFORM_LINUX>
      $<$<PLATFORM_ID:Darwin>:ARGUS_PLATFORM_MACOS>
      $<$<CONFIG:Debug>:ARGUS_DEBUG_MODE>
      $<$<CONFIG:RelWithDebInfo>:ARGUS_RELEASE_WITH_DEBUG_INFO_MODE>
      $<$<CONFIG:Release>:ARGUS_RELEASE_MODE NDEBUG>
  )
...
function(argus_target_set_shared_lib_platform TARGET)
  if(WIN32 AND MSVC)
    set_target_properties(${TARGET} PROPERTIES
...
function(argus_target_set_output_dirs TARGET)
  cmake_parse_arguments(ARG "" "CUSTOM_FOLDER;MODULE" "" ${ARGN})
  ...
  if(ARG_CUSTOM_FOLDER)
    set(_output_dir "${PROJECT_SOURCE_DIR}/bin/${ARG_CUSTOM_FOLDER}/$<LOWER_CASE:$<CONFIG>>-$<LOWER_CASE:${CMAKE_SYSTEM_NAME}>-$<LOWER_CASE:${CMAKE_SYSTEM_PROCESSOR}>")
  else()
    set(_output_dir "${PROJECT_SOURCE_DIR}/bin/$<LOWER_CASE:$<CONFIG>>-$<LOWER_CASE:${CMAKE_SYSTEM_NAME}>-$<LOWER_CASE:${CMAKE_SYSTEM_PROCESSOR}>")
  endif()

  if(ARG_MODULE)
    set(_output_dir "${_output_dir}/${ARG_MODULE}")
  endif()
...
function(argus_target_set_cxx_standard TARGET)
...
# Set C standard for a target
function(argus_target_set_c_standard TARGET)
  cmake_parse_arguments(ARG "" "STANDARD" "" ${ARGN})
...
function(argus_target_set_folder TARGET FOLDER_NAME)
...
function(argus_target_enable_unity_build TARGET)
...
function(argus_target_add_pch TARGET PCH_FILE)
...
function(argus_target_mark_dependencies_as_system TARGET)
...
# Usage: argus_target_link_system_libraries(my_target PUBLIC external::lib1 external::lib2)
function(argus_target_link_system_libraries TARGET VISIBILITY)
...
# Usage: argus_copy_runtime_dependencies(my_target LIBRARIES lib1 lib2...)
function(argus_copy_runtime_dependencies TARGET)
